...
public class OptionsWidgetParametersPopulateStrategy extends OptionsWidgetPopulateStrategy {

	public OptionsWidgetParametersPopulateStrategy(OptionsWidget widget, HttpServletRequest request) {
		super(widget, request);
	}

	public List<? extends Selectable> getOptions() {
		String populationValue = widget.getPopulationValue();
		String beanName = "widgetService";
		String methodName = "getOptionList";
		if (StringUtils.isNotBlank(populationValue) && populationValue.contains("|")) {
		...
		try {
			resultList = (List<Selectable>) MethodUtils.invokeMethod(obj, methodName,
					new Object[] { getMethodInvocationParameter() });
		} catch (Exception e) {
			e.printStackTrace();
		}
