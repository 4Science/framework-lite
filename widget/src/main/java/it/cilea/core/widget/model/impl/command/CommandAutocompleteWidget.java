...
@Entity
@DiscriminatorValue("command-autocomplete")
public class CommandAutocompleteWidget extends OptionsWidget {
	@Transient
	...
	public List<Selectable> getCollection(HttpServletRequest request) {
		...
		try {
			...
			if (provider != null) {
				selectableList = (List<Selectable>) provider.getOptions(optionIds);
			}
