			...
			<c:set var="inputValue">			
				<c:forEach var="item" items="${collection}" varStatus="loop">
					...
					<c:forEach items="${inputValue}" var="valoreSingolo">
						<c:if test="${item.identifyingValue == valoreSingolo || (empty valoreSingolo && empty item.identifyingValue)}"><c:set var="optionSelected" value="true"/></c:if>
					</c:forEach>
...
<c:set var="content">
	<widget:checkboxCore modeType="${modeType}" collection="${collection}"
