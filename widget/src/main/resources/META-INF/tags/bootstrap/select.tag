...
<c:if test="${empty size}"><c:set var="size" value="1" /></c:if>
<c:if test="${empty autoDisplay}"><c:set var="autoDisplay" value="${true}" /></c:if>
...
<spring:bind path="${propertyPath}">
			...
			<c:set var="inputValue">			
				<c:forEach var="item" items="${collection}" varStatus="loop">
					...
					<c:if test="${not multiple}">
						<c:if test="${item.identifyingValue == inputValue}"><c:set var="optionSelected" value="true"/></c:if>						
					</c:if>
					<c:if test="${multiple}">
	...
	<c:set var="inputId" value="${fn:replace(inputId,']','__')}" />
	<%--
	autoDisplay di default è  true. Quindi viene aggiunto ".displayValue" in coda al property path in caso di visualizzazione in un dettaglio. 
	Per una questione di uniformità con il form elimino dalla costruzione dell'id del select il ".displayValue" in coda
	Questo impatta SOLO il naming degli id dei tag html 
	--%>
...
<c:set var="content">
	<widget:selectCore modeType="${modeType}" collection="${collection}"
		inputName="${inputName}" inputValue="${inputValue}" inputId="${inputId}"
		cssClass="${cssClass}" readonly="${readonly}"
		onchange="${onchange}" onkeyup="${onkeyup}" onmouseover="${onmouseover}"
		hideName="${hideName}" size="${size}" multiple="${multiple}" type="${type}"/>
</c:set>
