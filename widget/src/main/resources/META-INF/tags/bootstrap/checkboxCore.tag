	...
	<c:otherwise>
		<c:if test="${collection != null}">
			<c:forEach var="item" items="${collection}" varStatus="loop">
				...
				<c:if test="${(empty inputValue && empty item.identifyingValue)}"><c:set var="optionSelected" value="true"/></c:if>
				...
				<c:choose>
					<c:when test="${positioning eq 'horizontal'}">
						<div class="checkbox">
							<label>
								<input type="checkbox" <c:if test="${not hideName}"> name="${inputName}"</c:if> id="${inputId}_${item.identifyingValue}" value="${item.identifyingValue}" 
