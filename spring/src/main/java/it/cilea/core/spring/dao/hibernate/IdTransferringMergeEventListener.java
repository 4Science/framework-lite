...
/**
 * Extension of Hibernate's DefaultMergeEventListener, transferring the ids of
 * newly saved objects to the corresponding original objects (that are part of
 * the detached object graph passed into the <code>merge</code> method).
 * 
 * <p>
 ...
 * the current Hibernate Session. This is particularly useful for web
 * applications that might want to store an object graph and then render it in a
 * web view, with links that include the id of certain (potentially newly saved)
 * objects.
 * 
 * <p>
 * The merge behavior given by this MergeEventListener is nearly identical to
 * TopLink's merge behavior. See PetClinic for an example, which relies on ids
 * being available for newly saved objects: the <code>HibernateClinic</code> and
 * <code>TopLinkClinic</code> DAO implementations both use straight merge calls,
 * with the Hibernate SessionFactory configuration specifying an
