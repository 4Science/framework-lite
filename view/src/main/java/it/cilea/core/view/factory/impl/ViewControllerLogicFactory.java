	...
	public ControllerLogic getControllerLogic(String url) {
		...
		if (context.containsBean("controller:" + url)) {
			...
			if (ViewConstant.viewBuilderMap.containsKey(ViewUtil.correctIdentifier(controllerLogic.getViewName()))) {
				ViewBuilder viewBuilder = ViewConstant.viewBuilderMap
						.get(ViewUtil.correctIdentifier(controllerLogic.getViewName()));
			...
			if (ViewConstant.viewBuilderMap.containsKey(ViewUtil.correctIdentifier(StringUtils.substringBeforeLast(url, ".")))) {
				ViewBuilder viewBuilder = ViewConstant.viewBuilderMap.get(ViewUtil.correctIdentifier(StringUtils.substringBeforeLast(url, ".")));
