...
public abstract class AbstractConnectorConfig {
	...
	protected boolean isAllowedCommon(File dir, FileActionEnum action) {
		...
		String rootPathString = getRootFile().getPath();
		if (dir.getPath().equals(rootPathString)) {
			if (action.equals(FileActionEnum.DELETE)) {
				return false;
			}
		} else if (dir.getParentFile().getPath().equals(rootPathString)) {
			if (action.equals(FileActionEnum.DELETE)) {
				return false;
			}
		}
		...
		boolean actionAllowed = getFs().isAllowedFile(dir, action);
	 ...
	 * @return
	 */
	public abstract String getMime(File file);
