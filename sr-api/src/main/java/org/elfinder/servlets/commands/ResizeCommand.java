...
public class ResizeCommand extends AbstractCommandOverride {
	...
	private static Logger logger = LoggerFactory.getLogger(ResizeCommand.class);
        ...
        if (dirCurrent != null) {
            ...
            if (mode.equals("resize")) {
                ...
                try {
					...
					BufferedImage rescaledImage = Scalr.resize(origImage, Method.SPEED, Mode.FIT_EXACT, with, height);
                    ImageIO.write(rescaledImage, ext, fileTarget);
                } catch (IOException rescaleEx) {
					logger.error("", rescaleEx);
                }
