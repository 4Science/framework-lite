...
import it.cilea.core.search.strategy.impl.hibernate.HibernateSearchStrategyData.DistinctType;
import it.cilea.core.search.util.SearchUtil;

@Repository("hibernateStrategyDao")
public class HibernateStrategyDaoHibernate implements HibernateStrategyDao {
	...
	private static final Logger log = LoggerFactory.getLogger(HibernateStrategyDao.class);
	...
	public String convertHqlToSql(String hqlQueryText) {

		if (hqlQueryText != null && hqlQueryText.trim().length() > 0) {
			hqlQueryText = SearchUtil.unescapeBracket(hqlQueryText);
			QueryTranslatorFactory translatorFactory = new ASTQueryTranslatorFactory();
			SessionFactoryImplementor factory = (SessionFactoryImplementor) sessionFactory.getCurrentSession()
					.getSessionFactory();
			QueryTranslator translator = translatorFactory.createQueryTranslator(hqlQueryText, hqlQueryText,
	...
	public Long getResultCount(HibernateSearchStrategyData data) {
		String queryString = null;

		if (data.isObjectsList()) {
			// query senza model class (con select o group widget)
			queryString = getQueryString(data, QueryType.COUNT_QUERY_OBJECTS);
		} else if (DistinctType.NO_DISTINCT_CLAUSE.equals(data.getDistinctType())) {
			queryString = getQueryString(data, QueryType.COUNT_QUERY);
		} else if (DistinctType.DISTINCT_CLAUSE.equals(data.getDistinctType())) {
			queryString = getQueryString(data, QueryType.DISTINCT_COUNT_QUERY);
		} else if (DistinctType.DISTINCT_CLAUSE_BLOB.equals(data.getDistinctType())) {
			queryString = getQueryString(data, QueryType.DISTINCT_COUNT_ID_QUERY);
		}

		if (data.isObjectsList()) {
			// devo trovare la query sql e poi lanciarla con select count(*)
			// from
			String querySql = convertHqlToSql(queryString);
			querySql = "select count(*) from (" + querySql + ")a";
			querySql = SearchUtil.unescapeBracket(querySql);
			Query query2 = sessionFactory.getCurrentSession().createSQLQuery(querySql);
			return ((Number) query2.uniqueResult()).longValue();
		}
	...
	public List<?> getResultList(HibernateSearchStrategyData data) {
		...
		if (data.isObjectsList()) {
			...
			throw new IllegalStateException(
					"When using HibernateStrategyData MUST BE specified the DistinctType to use: NO_DISTINCT_CLAUSE, DISTINCT_CLAUSE, DISTINCT_CLAUSE_BLOB");
		...
		if (DistinctType.DISTINCT_CLAUSE_BLOB.equals(data.getDistinctType())) {
			...
			if (resultList.size() < 1000) {
				...
				for (int i = 0; i < nIteration; i++) {
					...
					// metto nell'oggetto HibernateSearchStrategyData la
					// porzione di lista
	...
	public String getSqlList(HibernateSearchStrategyData data) {
		...
		if (data.isObjectsList()) {
			...
			throw new IllegalStateException(
					"When using HibernateStrategyData MUST BE specified the DistinctType to use: NO_DISTINCT_CLAUSE, DISTINCT_CLAUSE, DISTINCT_CLAUSE_BLOB");
		return convertHqlToSql(queryString);
	}

	private String getLeftJoinClause(HibernateSearchStrategyData data, QueryType queryType) {
		...
		for (String modelAttributeName : modelAttributeNameList) {
			HibernateModelAttributeNameWrapper wrapper = new HibernateModelAttributeNameWrapper(modelAttributeName);
		...
		if (StringUtils.isNotBlank(data.getWithClause()))
			for (String with : StringUtils.splitByWholeSeparator(data.getWithClause(), "___")) {
				String prefix = StringUtils.substringBefore(with, ":");
				...
				for (String join : joinSet) {
					if (join.endsWith(prefix))
						joinSetNew.add(join + " with " + suffix);
					else
						joinSetNew.add(join);
				}
	...
	private String getWhereClause(HibernateSearchStrategyData data, QueryType queryType) {
	...
	private String getOrderClause(HibernateSearchStrategyData data) {
	...
	private String getGroupingClause(HibernateSearchStrategyData data) {
		...
		for (String string : groupParameterSet) {
			groupFinale = StringUtils
					.replace(groupFinale, "{" + string + "}", SearchUtil.getAliasedSelectValue(string));
		}
	...
	private String getQueryString(HibernateSearchStrategyData data, QueryType queryType) {
		...
		if (!(QueryType.COUNT_QUERY.equals(queryType) || QueryType.DISTINCT_COUNT_QUERY.equals(queryType) || QueryType.DISTINCT_COUNT_ID_QUERY
				.equals(queryType))) {
